# Архитектура проекта
В проекте используется паттерн **Event-Driven Architecture (EDA)**.
Связь между слоями реализована через событийный брокер EventEmitter, который инкапсулирует подписку и генерацию событий.
Это позволяет модулям взаимодействовать только через события, не зная напрямую друг о друге.
Центральным звеном является глобальный EventEmitter, через который компоненты общаются друг с другом.
